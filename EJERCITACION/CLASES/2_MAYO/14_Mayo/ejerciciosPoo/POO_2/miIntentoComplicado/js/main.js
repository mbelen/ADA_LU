/*
Listo, PERO:
- Se podría rehacer viendo que las opciones dietarias no se excluyan mutuamente (¿checkboxes?)
- Si elijo como omnívoro, no me dice qué clase de plato elegí (porque toma los inputs)
- Ver si se puede rehacer simplificado. Estoy segura de que se puede reescribir en un tercio de las líneas
- Tengo que repasar cómo appendear opciones a un select dinámicamente
...
var optionDefault = `<option>Elegí una opción</option>`
function cargarEntradas(){
	if(entradaTodas.is(":checked")){
		$("#selectEntrada").children().remove();
		$("#selectEntrada").append(optionDefault);
...
function cargarPrincipales(){
	if(principalTodas.is(":checked")){
		...
		$("#selectPpal").append(optionDefault);
...
function cargarPostres() {
	if(postreTodas.is(":checked" ) && $("input[value='nolight']").is(":checked")){
		...
		$("#selectPostre").append(optionDefault);
...
function cargarBebidas(){
	...
	if($("input[value='sialcohol']").is(':checked')){
		...
		$("#selectBebidas").append(optionDefault);
	...
	}else if($("input[value='sialcohol']").is(':checked')){
		$("#selectBebidas").children().remove();
		for(i=0;i<bebidas.length;i++){
			if(bebidas[i].alcohol==="si"){
			var option = `<option>${bebidas[i].name}</option>`;
...
$("#enviar").on('click', function crear() {
	if($("#selectEntrada :selected").val()==="Elegí una opción"||$("#selectPpal :selected").val()==="Elegí una opción"||$("#selectEntrada :selected").val()==="Elegí una opción"||$("#selectEntrada :selected").val()==="Elegí una opción"||$("#selectPostre :selected").val()==="Elegí una opción"||$("#selectBebida :selected").val()==="Elegí una opción"){
		$("#error").text("¡Debes seleccionar todas las opciones!")
	}else{
	$("#error").text("")	
	...
	if($("input[value='silight']").is(':checked')){
		postri._lightness = true;
	}else if($("input[value='nolight']").is(':checked')){
		postri._lightness = false;
	}
	...
	if($("input[value='sialcohol']").is(':checked')){
		bebida._alcohol = true;
	}else if($("input[value='noalcohol']").is(':checked')){
		bebida._alcohol = false;
	}
	console.log(bebida)
	var menu = new Menu;
